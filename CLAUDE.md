...
Mirrors files from Caddy file servers with `file_server { browse }` enabled. Three implementations: `mirror-caddy.sh` (bash), `mirror-caddy.py` (Python asyncio), and `rust/` (Rust). All enumerate directories via Caddy's JSON API, download files in parallel, and cache ETag/Last-Modified headers for incremental updates (HTTP 304).
...
# Rust version
cd rust && cargo run --release -- https://example.com ./output
cd rust && cargo run --release -- -v https://example.com ./output  # debug
cd rust && cargo run --release -- -vv https://example.com ./output # trace

# All versions support PARALLEL_JOBS environment variable
PARALLEL_JOBS=16 ./mirror-caddy.sh https://example.com ./output
PARALLEL_JOBS=16 ./mirror-caddy.py https://example.com ./output
PARALLEL_JOBS=16 cargo run --release -- https://example.com ./output
```

### Exit Codes

- `0` - Success (all files downloaded or already up-to-date)
- `1` - Failure (some files failed to download)
...
All implementations share the same two-phase data flow:
...
**Rust**: Uses `tokio` async runtime with `reqwest` for HTTP. Channel-based work queue for enumeration (N workers pull from shared queue, deadlock-free). Downloads limited by semaphore (default 50 concurrent). Includes retry logic with exponential backoff (3 attempts: 500ms, 2s, 5s).

### Metadata Cache

All three implementations cache ETag and Last-Modified headers in `.metadata/*.meta` files for incremental updates (HTTP 304 Not Modified).

## Dependencies

- Bash version: `bash` (4+), `curl`, `jq`
- Python version: Python 3.7+, `aiohttp` (`pip install aiohttp`)
- Rust version: Rust toolchain, `cargo` (dependencies in `rust/Cargo.toml`)
